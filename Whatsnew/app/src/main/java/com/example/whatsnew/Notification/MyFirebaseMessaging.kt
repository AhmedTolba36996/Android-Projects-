...
class MyFirebaseMessaging : FirebaseMessagingService()
{
    ...
    override fun onMessageReceived(mRemoteMessage: RemoteMessage) {
        ...
        val cureentOnlineUser = sharedPref.getString("currentUser","name")

        val firebaseUser = FirebaseAuth.getInstance().currentUser

        if(firebaseUser!=null && sented==firebaseUser.uid)
        {
            if(cureentOnlineUser != user)
            {
    ...
    private fun SendNotification(mRemoteMessage: RemoteMessage)
    {
        ...
        val j = user!!.replace("[\\D]".toString() , "").toInt()
    ...
    private fun SendOreoNotification(mRemoteMessage: RemoteMessage)
    {
        ...
        val oreNotification = OreoNotification(this)

        val builder:Notification.Builder = oreNotification.getOreoNotification(title,body,pendingIntent,defaultSound,icon)
